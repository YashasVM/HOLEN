...
class OutputStore(private val context: Context) {
    ...
    suspend fun publish(
        ...
        val tree = treeUri ?: throw IOException("Download folder permission is missing.")
        if (!hasValidTreeGrant()) throw IOException("Download folder permission was revoked.")
        ...
        val document = DocumentsContract.createDocument(
            ...
            staged.mimeType,
            safeName,
        ) ?: throw IOException("The selected folder could not create a file.")
        ...
        try {
            ...
            } ?: throw IOException("The selected folder could not be written.")
            if (copied != staged.file.length()) {
                throw IOException("The copied file did not match the completed download.")
            }
    ...
    suspend fun cleanOrphanStaging(now: Long = System.currentTimeMillis()) =
        withContext(Dispatchers.IO) {
            ...
            root.listFiles()
                ?.filter { it.isDirectory && now - it.lastModified() > ORPHAN_MAX_AGE_MS }
