...
## In progress
- Audit the aria2 external-downloader path before changing retry/timeout behavior. Upstream yt-dlp's `Aria2cFD` does not translate yt-dlp's generic `retries` or socket-timeout settings into aria2 arguments; aria2 therefore keeps its own retry/timeout defaults for ordinary HTTP(S) transfers unless HOLEN passes explicit `aria2c:` downloader args. A key policy detail is now confirmed: yt-dlp `--retries 3` means three retries after the initial attempt, while aria2 `--max-tries` counts total attempts, so an aligned aria2 budget is `--max-tries=4`, not 3. Validate the exact downloader args with a controlled external-downloader timeout/drop test before wiring them into production.
