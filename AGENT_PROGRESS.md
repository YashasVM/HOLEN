...
## Evidence / validation
- Aria2 policy/retry work, cookie hardening, deferred startup update checks, cookie-state cache generation, HTTP 416 guidance, numbered fragment-temp filtering, and HTTP 503 `Retry-After` scheduling all have green generic + Android CI from their final code tips.
...
## Next review target
- Audit the next evidence-backed Android production bottleneck, prioritizing first-download latency, duplicate staging/disk work, or reliability regressions from current yt-dlp/Android upstream changes. Do not tune fragment concurrency or remove the SAF collision scan without representative measurements or a crash-safe design.
