...
## In progress
- Establish a defensible app-startup baseline before changing startup behavior. Live logs from Android CI `33676474347` showed the emulator action invoked its `script:` through `/bin/sh`, where `set -euo pipefail` fails before any instrumentation test starts. Commit `f2f32467` switches only that action script to POSIX-compatible `set -eu`; the existing post-pipeline `test`/`grep` checks still enforce timing evidence. The rendered-home test writes `app_home_ms` into the debug app's files directory and CI reads it back with `run-as`. No production startup optimization has been made or claimed yet.
...
- Startup harness attempts through `33676474347` have not yet yielded a defensible launch-to-home baseline. The latest failure was a CI shell-portability error before tests executed, not app performance evidence.
