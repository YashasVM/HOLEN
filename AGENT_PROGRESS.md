...
## In progress
- Stale-extractor recovery still performs no updater or automatic retry action. Current candidate detection requires yt-dlp's explicit `please report this issue` guidance plus `unable to extract`/`failed to extract`; generic `No video formats found` is excluded even with bug-report guidance.
- Added a deadlock-safe recovery sequencer that requires the failed normal operation to return before maintenance runs, then allows exactly one retry. It preserves the original extractor failure if refresh itself fails and never recovers cancellations or non-candidate errors. The helper is intentionally not wired into `YtDlpEngine.analyze()` until its new unit coverage and Android CI pass.
