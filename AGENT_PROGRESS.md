...
- Android CI `34398224378`: normal instrumentation, lint/test/build, and strict 16 KB verification passed after making rate-limited opt-in EJS validation runs fail instead of skip.
- Generic CI `34398224384` and `34398275355`: passed for the strict EJS block-handling change and its progress update.
...
- Upstream youtubedl-android PR #350 remains the pinned source for the temporary arm64 16 KB wrapper fix; re-check upstream before replacing it.
