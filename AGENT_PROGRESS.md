...
- Added a strict stale-extractor candidate classifier with exclusion-focused unit coverage. Android CI `33838498927` passed before the classifier was tightened further using current upstream false-positive evidence.

## In progress
- Stale-extractor recovery still performs no updater or automatic retry action. Current candidate detection requires yt-dlp's explicit `please report this issue` guidance plus `unable to extract`/`failed to extract`; generic `No video formats found` is now excluded even with bug-report guidance because current upstream cases show the same terminal text for cookie/live-stream combinations that an engine refresh cannot reliably fix.
- Before wiring automatic recovery, restructure analysis so an extractor failure fully exits `EngineOperationGate.withOperation` before calling exclusive `updateStable()`; attempting maintenance from inside an active operation would deadlock by waiting for itself to drain.
