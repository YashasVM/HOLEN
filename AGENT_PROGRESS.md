...
- Added a durable per-job authentication-policy sidecar for the explicit no-cookie recovery path. Missing or unknown state fails safe to `CONFIGURED`, `WITHOUT_COOKIES` survives process/service restarts, and stale entries can be pruned without changing the core jobs schema. JVM coverage checks fail-safe decoding and job-key validation.

## In progress
- Finish the Android-only “retry without cookies” path by wiring the persisted policy into yt-dlp execution and the failed-job UI action. The action must preserve the selected semantic format, be offered only through the existing eligibility gate, and never silently switch private/age-restricted/members-only jobs to unauthenticated access.
...
- Persisted authentication-policy commits `43dc685b`, `2beb3576`, `e29ab2c8`: Android CI run `33606011186` and generic CI run `33606011204` are pending.
...
- YouTube account cookies rotate and upstream recommends using them only for content that actually needs authentication. HOLEN still applies a configured cookie file to yt-dlp requests. The durable sidecar now solves restart-safe policy persistence, but production execution/UI wiring is intentionally not enabled until the new persistence code passes CI.
- The authentication-policy sidecar deliberately uses absence as `CONFIGURED`, preserving every existing job and avoiding a database migration for one recovery bit. Lifecycle cleanup must remain wired when the user-facing recovery action is added so removed jobs do not leave stale entries indefinitely.
