...
for apk in "$@"; do
  ...
  while IFS= read -r so; do
    ...
    if is_wrapper_zip_payload "$so"; then
      payload_dir="$(mktemp -d "$tmp/payload.XXXXXX")"
      if ! unzip -qq "$so" -d "$payload_dir"; then
        echo "$apk contains wrapper payload ${relative} that could not be extracted." >&2
        exit 1
      fi
      ...
      while IFS= read -r payload_file; do
        ...
        verify_elf_alignment \
          "$payload_file" \
          "$apk contains 64-bit ELF ${payload_relative} inside wrapper payload ${relative}" \
          "$tmp/program-headers.txt" || exit 1
