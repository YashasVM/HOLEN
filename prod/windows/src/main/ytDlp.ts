...
import { engineDir, cookiesPath, prefsStore, getPrefs } from './store.js'
...
function ytDlpBin(): string {
  // In packaged app, yt-dlp.exe + ffmpeg.exe are in resources/bin
  const resBin = app.isPackaged
...
export function isYtDlpAvailable(): boolean {
  try { return existsSync(ytDlpBin()) } catch { return false }
}
...
async function runYtDlp(args: string[], timeoutMs = 45_000): Promise<{ stdout: string; stderr: string; code: number | null }> {
  return new Promise((resolve, reject) => {
    ...
    let out = '', err = ''
    const timer = setTimeout(() => { try { child.kill() } catch {} ; reject(new Error('yt-dlp timed out')) }, timeoutMs)
...
export async function getEngineVersion(): Promise<string> {
  ...
  } catch { return getPrefs().engineVersion || 'Bundled (yt-dlp)' }
}
...
export async function analyze(url: string, mode: AnalysisMode = 'FULL'): Promise<SourceAnalysis> {
  ...
  let json: any
  try { json = JSON.parse(stdout) } catch { throw new Error('Could not parse video metadata. Try a different link.') }
...
function estimateSize(formats: any[], target: DownloadFormat): number | null {
  ...
  // naive: best match by ext/height, then filesize
  let best: any = null
  for (const f of formats) {
    const h = f.height as number | undefined, ext = (f.ext as string) || ''
    let score = -1
    ...
    if (score >= 0 && (!best || score > best._score || (score===best._score && (f.filesize??0) > (best.filesize??0)))) { best = { ...f, _score: score } }
  }
  if (!best) return null
...
function downloadArgs(format: DownloadFormat): string[] {
  switch (format) {
    case 'ORIGINAL': throw new Error('Original handled by direct downloader')
    case 'BEST_MP4': return ['-f','bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best','--merge-output-format','mp4']
    case 'MP4_1080': return ['-f','bestvideo[height<=1080][ext=mp4]+bestaudio[ext=m4a]/best[height<=1080][ext=mp4]/best[height<=1080]','--merge-output-format','mp4']
    case 'MP4_720': return ['-f','bestvideo[height<=720][ext=mp4]+bestaudio[ext=m4a]/best[height<=720][ext=mp4]/best[height<=720]','--merge-output-format','mp4']
    case 'AUDIO_M4A': return ['-f','bestaudio[ext=m4a]/bestaudio','--extract-audio','--audio-format','m4a']
    case 'AUDIO_MP3': return ['-f','bestaudio','--extract-audio','--audio-format','mp3','--audio-quality','0']
  }
}
...
export function downloadMedia(
  ...
  return new Promise((resolve, reject) => {
    ...
    const suffixEnabled = prefsStore.get('filenameSuffixEnabled') !== false
    ...
    const args = [
      ...
      '--continue','--windows-filenames','--no-overwrites','--embed-metadata',
      '--concurrent-fragments','2','--retries','3','--fragment-retries','3','--socket-timeout','20',
      '--progress','--progress-template', `download:${PROGRESS_MARKER} %(progress._percent_str)s|%(progress.downloaded_bytes)s|%(progress.total_bytes)s|%(progress.total_bytes_estimate)s|%(progress.speed)s|%(progress.eta)s`,
      '--newline','--no-playlist',
      '--output', outTpl,
    ...
    const onData = (chunk: Buffer) => {
      ...
      stderr += text
      // yt-dlp progress goes to stderr merged; parse each line
      for (const line of text.split('\n')) {
        ...
        if (l.includes(PROGRESS_MARKER)) {
          ...
          if (p) { lastProgress = merge(p, lastProgress); maybeEmit(p) }
    ...
    function maybeEmit(p: TransferProgress) {
      const now = Date.now()
      if (now - lastEmit >= 250 || p.percent >= 100) {
        onProgress(p); lastEmit = now
      }
    }
    ...
    child.on('close', (code) => {
      ...
      // prefer printed filepath
      const printed = stdout.split('\n').map(s=>s.trim()).filter(s=> s && (s.includes('\\')||s.includes('/'))).pop()
      let file: string | null = null
      if (printed && existsSync(printed)) file = printed
      else {
        // fallback: newest file not in before set
        const nowFiles = readdirSync(stagingDir).map(n=> join(stagingDir,n)).filter(p=> !before.has(p) && statSync(p).isFile())
        if (nowFiles.length) file = nowFiles.sort((a,b)=> statSync(b).mtimeMs - statSync(a).mtimeMs)[0]
        else {
          const all = readdirSync(stagingDir).map(n=> join(stagingDir,n)).filter(p=> statSync(p).isFile()).sort((a,b)=> statSync(b).mtimeMs - statSync(a).mtimeMs)[0]
          if (all) file = all
        }
      }
    ...
    // poll cancellation
    const iv = setInterval(() => { if (isCancelled()) { try { child.kill() } catch {}; clearInterval(iv) } }, 250)
...
function parseMachineLine(line: string): TransferProgress | null {
  ...
  const n = (s: string) => { const t=s.trim(); if(!t||t.toUpperCase()==='NA') return null; const v=Number(t); return Number.isFinite(v)? v:null }
